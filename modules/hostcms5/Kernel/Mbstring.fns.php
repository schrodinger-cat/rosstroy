 ...
 * Модуль Kernel.
 *
 * Файл: /modules/Kernel/Mbstring.fns.php
 *
 * @package HostCMS 5
 * @author Hostmake LLC
 * @version 5.x
...
if (!function_exists('mb_substr'))
{
	...
	function mb_substr($str, $start, $length = null, $encoding = null)
	{
		...
		$real_len = mb_strlen($str);

		if ($len != null && $len > $real_len)
		{
			$len = $real_len - $from;
		}
		...
		if ($from >= 0)
		{
			for ($c_byte = 0; $from--; )
			{
				if (isset($str[$c_byte]) && ord($str[$c_byte]) <= 0x7F)
				{
					$c_byte++;
				}
				else
				{
					while (($t = ++$c_byte) && isset($str[$t])
					&& (ord($str[$t]) & 0xc0) == 0x80);
				}
			}
...
if (!function_exists('mb_strpos'))
{
	...
	{
		$offset = ($offset < 0 || $offset == null) ? 0 : intval($offset);

		if ($offset > 0)
		{
			preg_match("/^.{{$offset}}(.*)/su", $haystack, $tmp);
			$haystack = isset($tmp[1]) ? $tmp[1] : '';
		}

		$p = strpos($haystack, $needle);

		// Область поиска пуста или поиск strpos вернул false
		if ($haystack == ''
		|| $p === false)
		{
			return false;
		...
		// Текущая позиция равна смещению
		$r = $offset;
...
if (!function_exists('mb_strrpos'))
{
	...
	{
		// Текущее найденное положение последнего символа
		$pos = false;

		if ($offset > 0 || $offset == null)
		{
			// Ищем каждое первое вхождение,
			// если нашли, увеличиваем $offset до следующего после найденного
			while (($offset = mb_strpos($haystack, $needle, $offset)) !== false)
			{
				$pos = $offset;
				// Ищем со следующей позиции
				$offset++;
			}
		}
		else
		{
			...
			// $offset отрицательный, получается вычитание
			$offset = $strlen + $offset;

			// В строке еще будут символы
			if ($offset > 0)
			{
				// Отрезаем строку
				$haystack = mb_substr($haystack, 0, $offset);

				while (!($pos = mb_strpos($haystack, $needle, $offset)))
				{
					// Ищем с предыдущей позиции
					$offset--;
				}
			}
		}
...
if (!function_exists('mb_substr_count'))
{
	...
	function mb_substr_count($haystack, $needle, $encoding = null)
	{
		...
		preg_match_all("/{$needle}/u", $haystack, $tmp);
