...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="helpdesk">
		...
		<h1>Служба поддержки <xsl:value-of disable-output-escaping="yes" select="name"/></h1>
							...
							<xsl:otherwise>
								...
								<xsl:choose>
									...
									<xsl:otherwise>
										...
										<xsl:if test="expire_after_days/node()"><xsl:text> </xsl:text><xsl:value-of disable-output-escaping="yes" select="expire_after_days"/><xsl:text> </xsl:text>
											...
											<xsl:otherwise>
											<xsl:if test="expire_after_hours/node()"><xsl:text> </xsl:text><xsl:value-of disable-output-escaping="yes" select="expire_after_hours"/><xsl:text> </xsl:text>
											...
											<xsl:if test="expire_after_minutes/node()"><xsl:text> </xsl:text><xsl:value-of disable-output-escaping="yes" select="expire_after_minutes"/><xsl:text> </xsl:text><xsl:variable name="nominative">минуты</xsl:variable>
