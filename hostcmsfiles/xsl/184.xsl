...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="helpdesk_ticket">
		<h1>
			<xsl:value-of disable-output-escaping="yes" select="helpdesk_message/subject"/><xsl:text> — [</xsl:text><xsl:value-of disable-output-escaping="yes" select="number"/>]
		</h1>
	...
	<xsl:template match="helpdesk_message">
		<div id="{@id}" class="comment">
			<div class="subject"><xsl:value-of disable-output-escaping="yes" select="subject"/></div>
		...
		<div class="comment_reply" id="cr_{@id}">
			<xsl:call-template name="AddReplyForm">
				...
				<xsl:with-param name="message_comment_subject">Re: <xsl:value-of select="subject" disable-output-escaping="yes" /></xsl:with-param>
			</xsl:call-template>
		</div>
