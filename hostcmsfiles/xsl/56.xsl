...
<xsl:stylesheet version="1.0"
	...
	<div class="shop_property">Вес товара: <span hostcms:id="{@id}" hostcms:field="weight" hostcms:entity="shop_item"><xsl:value-of select="weight"/></span><xsl:text> </xsl:text><span><xsl:value-of select="/shop/shop_measure/name"/></span></div>
		...
		<xsl:if test="count(modifications/shop_item) &gt; 0">
			<p class="h2">Модификации <xsl:value-of disable-output-escaping="yes" select="name"/></p>
			<ul class="shop_list">
		...
		<xsl:if test="count(associated/shop_item) &gt; 0">
			<p class="h2">Сопутствующие товары <xsl:value-of disable-output-escaping="yes" select="name"/></p>
			<ul class="shop_list">
...
<div class="shop_discount"><xsl:value-of disable-output-escaping="yes" select="name"/><xsl:text> </xsl:text><span><xsl:value-of select="percent"/>%</span></div>
	</xsl:template>
