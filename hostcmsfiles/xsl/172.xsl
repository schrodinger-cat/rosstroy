				...
				<div class="comment" style="display: none" id="AddItemForm">
					...
					<xsl:for-each select="shop_item_properties//property">
						...
						<div class="row">
							<div class="caption">
								<xsl:value-of disable-output-escaping="yes" select="name" />
								<xsl:if test="shop_measure/node()">(<xsl:value-of select="shop_measure/name" disable-output-escaping="yes" />)</xsl:if>
							</div>
							<div class="field">
			...
			<td>
				...
				<xsl:choose>
					<xsl:when test="price != 0">
						<xsl:value-of disable-output-escaping="yes" select="price"/><xsl:text> </xsl:text><xsl:value-of disable-output-escaping="yes" select="currency"/>
					</xsl:when>
