...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="helpdesk">
		...
		<h1>Режим работы службы поддержки <xsl:value-of disable-output-escaping="yes" select="name"/></h1>

		<p>
			<a href="{url}">Служба поддержки <xsl:value-of disable-output-escaping="yes" select="name"/></a>
