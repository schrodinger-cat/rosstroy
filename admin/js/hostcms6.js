(function(jQuery){
	...
	jQuery.fn.extend({
		...
		{
			return this.each(function(){
				//var object = jQuery(this), bChecked = object.find(" input[type='checkbox'][id^='check_']:not(:checked)").length === 0;
	...
	jQuery.extend({
		...
		adminCheckObject: function(settings) {
			...
			if (cbItem.length > 0)
			{
				...
				jQuery("#"+settings.windowId+" input[type='checkbox'][id^='check_']:not([name*='_fv_'])").attr('checked', false);
		...
		ajaxCallback: function(data, status, jqXHR)
		{
			...
			{
				if (jObject.attr('id') == 'id_content')
				{
					document.title = data.title;
				}
				//else

				if (jObject.hasClass('hostcmsWindow'))
				{
		...
		{
			jQuery("#" + windowId + " .admin_table_filter").find('input,select').val('');
		},
		...
		{
			var jSpecialPrice = jQuery(cloneDelete).parent('.spec_prices'),
		...
		{
			var jObject = jQuery(object).parent('.spec_prices').remove();
		},
