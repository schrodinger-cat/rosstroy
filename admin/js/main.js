	...
	if (typeof CodePress != 'undefined')
	{
		...
		else
		{
			...
			for (var i = 0; i < textarea_array.length; i++)
			{
				...
				{
					var id = textarea_array[i].id, old_id = id.split('_cp').join('');
...
function cSelectFilter(windowId, sObjectId)
{
	...
	this.Filter = function() {
		...
		var jImg = jQuery('<img>').addClass('img_line selestFilterAjax').prop('src', '/admin/js/ui/themes/base/images/ajax-loader.gif');
		$(this.oCurrentSelectObject).after(jImg), self = this;
